...
4. **Modül 5**'te yer alan `simple_agent.py` dosyasını çalıştırarak (`make run-client`) Claude Desktop'a hiç ihtiyaç duymadan, kendi yazdığınız saf bir Python botunun bir MCP sunucusuna nasıl bağlandığını analiz edin. Kendi "Host"unuzu (Ajan Beynini) yazmanın temelleri buradadır.
5. **Büyük Final (Modül 6):** Asıl otonomi zirvesine ulaşmak için `make run-llm` (veya `src/06_llm_agent/autonomous_brain.py`) kodunu çalıştırın. Burada Anthropic API'sinin, MCP sunucularını nasıl kendi kendine kullanmaya karar verdiğini (**ReAct Döngüsü**) görerek Agentic AI mantığının şifresini tamamen çözeceksiniz.
